 ...
 */
package pro.buildmysoftware.efficientjava.review.memleak;
...
public class Leak {
	...
	public static void main(String[] args) throws IOException {
		Hashtable<DemoObject, String> h = new Hashtable<>();
		Thread[] threads;
		int leakspeed = 1;
		int threadCount;
		...
		System.out.println(String
			.format("Starting leak with %d threads and a leak " +
				"speed of %d", threadCount, leakspeed));
	...
	private static void startAllocThread() {
		Thread thread = new Thread(new TransientAllocator(),
			"Transient Allocator");
	...
	public static void startLeak() {
		Hashtable<DemoObject, String> h = new Hashtable<>();
		Thread t = new Thread(new DemoThread(h, 1), "Leaking " +
			"Allocator");
		t.start();
	}
	...
	public static void startFastLeak() {
		Hashtable<DemoObject, String> h = new Hashtable<>();
		Thread t = new Thread(new DemoThread(h, 2), "Fast Leaking " +
			"Allocator");
		t.start();
	}
	...
	public static void startNonLeak() {
		Hashtable<DemoObject, String> h = new Hashtable<>();
		Thread t = new Thread(new DemoThread(h, 0), "Not Leaking " +
			"Allocator");
		t.start();
	}
	...
	private static class DemoObject {
		...
		public boolean equals(Object o) {
			return (o instanceof DemoObject) && (o
				.hashCode() == position);
		}
	}
	...
	private static class TransientAllocator implements Runnable {
		...
		public void run() {
			while (true) {
				// Alloc transients
				List<Object> junkList = new ArrayList<>();
				for (int i = 0; i < 1000; i++) {
					junkList.add(new Object());
					for (int j = 0; j < 10; j++)
						// Keep busy yielding for a
						// little
						// while...
					{
						Thread.yield();
	...
	private static class DemoThread implements Runnable {
		...
		DemoThread(Hashtable<DemoObject, String> table,
			   int leakspeed) {
			this.table = table;
