 ...
 */
package pro.buildmysoftware.efficientjava.review.gc;
...
/**
 * Will allocate a map with ExampleMapContent, and then proceeds to check for
 * each ExampleMapContent
...
public final class Allocator implements Runnable {
	...
	public void run() {
		long yieldCounter = 0;
		while (true) {
			Collection<ExampleMapContent> myAllocSet =
				map.values();
