 ...
 */
package pro.buildmysoftware.efficientjava.review.hotmethods;

import java.io.IOException;

/**
 * Never mind this class. These are not the droi... This is not the class
 * you're looking for. It's
 * just for setting up the worker threads. The problem is elsewhere. ;)
 */
public class HotMethods {

	// Hint: You may want to set NUMBER_OF_THREADS close to the number of
	// hardware threads for maximum saturation
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < threads.length; i++) {
			threads[i] = new Thread(threadGroup, new Worker(),
				"Worker Thread " + i);
